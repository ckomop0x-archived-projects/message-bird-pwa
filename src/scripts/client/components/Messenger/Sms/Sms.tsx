...
const Sms = (props: SmsProps) => {
    const {apiKey, filter, sendNotification, socket, balance, isOffline, messagebird} = props;
    ...
    return (
        <>
            ...
            {props.filter === 'send' ? (
                ...
                <Dashboard filter={filter} sendNotification={sendNotification} socket={socket} apiKey={apiKey} />
            )}
        </>
    );
};
